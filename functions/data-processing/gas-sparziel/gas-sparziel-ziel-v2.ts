import { deleteAllGasSparzielZielV2, saveGasSparzielZielV2 } from '/opt/nodejs/db/gas-sparziel-ziel-v2.db';
...
const gasSparzielZiel = async () => {
    const records = await getCSVFileFromS3<SparzielZielSource>(SourceFiles.GAS_SPARZIEL_ZIEL_V2);
...
const validateRecords = (records: SparzielZielSource[]) => {
    if (records.length !== 2) {
        const errorMsg = `Expecting exactly 2 elemente for ${SourceFiles.GAS_SPARZIEL_ZIEL}, got ${records.length} elements`;
