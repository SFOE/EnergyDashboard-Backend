...
import { getYesterday, isDateBeforeOrEqualsToday, isSameDate } from '../utils/date.utils';
...
export const mapToApiModel = (records: StromVerbrauchLandesverbrauchMitPrognoseV2[]): StromVerbrauchLandesverbrauchMitPrognoseApiV2 => {
...
const getEntryYesterday = (records: StromVerbrauchLandesverbrauchMitPrognoseV2[]) => {
    const yesterday = getYesterday();
    return findEntry(records, yesterday);
}

const getEntryInFiveDays = (records: StromVerbrauchLandesverbrauchMitPrognoseV2[]) => {
    const date = new Date(new Date().getTime() + (5 * 24 * 60 * 60 * 1000));
    return findEntry(records, date);
}

const findEntry = (records: StromVerbrauchLandesverbrauchMitPrognoseV2[], date: Date): StromVerbrauchLandesverbrauchMitPrognoseV2 => {
    return records
        .find(entry => isSameDate(new Date(entry.date), date));
}

const mapCurrentEntry = (entry: StromVerbrauchLandesverbrauchMitPrognoseV2, entryInFiveDays: StromVerbrauchLandesverbrauchMitPrognoseV2): StromVerbrauchLandesverbrauchMitPrognoseCurrentEntryApiV2 => {
    return {
